...
$(function(){
	...
	var personNum=String(tplCfg.param.person_num);
	var personNumstr=personNum.split('');
	var personNumLength=String(personNum).length;   //长度
	
	console.log(personNumstr);
...
function initEvent(){
	...
	//选中选手  加载多次
	$('#masonry').unbind('click').on('click','.vote-btn',function(){
		if(tplCfg.param.rule_type === 1){   //单选
			...
			$(this).removeClass('vote-btn').addClass('vote-cancel').html('取消');
			$(this).parent('div').siblings('.player-info').addClass('choosed-vote');
			addgetCookie(_this);
		}
	...
	$('#masonry').on('click','.vote-cancel',function(){
		_this=$(this);
		$(this).removeClass('vote-cancel').addClass('vote-btn').html('选择');
		$(this).parent('div').siblings('.player-info').removeClass('choosed-vote');
	...
	$('.close-vote-bottom').click(function(){
		voteNumberDown();
	    $.cookie('selectPlayer', '', { expires: -1 });
	    $.cookie('chooseNumber', '', { expires: -1 });
		$('#masonry').find('.box').find('.player-info').removeClass('choosed-vote');
		$('#masonry .box .vote-cancel').removeClass('vote-cancel').addClass('vote-btn').html('选择');
		$('.click-vote').removeClass('true').addClass('false');
	});				
	...
	if($.cookie('selectPlayer') != undefined){
		...
		for(var i=0;i<length;i++){
			...
			if(oldData.indexOf(classRank) > -1){
				$('#masonry').find('.box').eq(i).find('.vote-btn').removeClass('vote-btn').addClass('vote-cancel').html('取消');
...
function addgetCookie(_this){
	var data=$.cookie('chooseNumber'); 
	var arr=_this.siblings('.Player-rank').children('span').attr('class').substr(4);
	var selectPlayer=$.cookie('selectPlayer');
	if(selectPlayer == undefined ){    //第一次创建
		$.cookie('selectPlayer',arr+',');
	}else{
...
//      obj.find('.player-info > .Player-name').attr('href', tplCfg.urls.url_vote_detail + '?id=' + item.id);
    ...
    if(tplCfg.param.hide_index_vote_btn)   obj.find('.vote-btn').hide();
    ...
    if (item.count !== undefined) obj.find('.Player-rank').html('<span class="cnt_' + item.id + ' font-big">' + item.count + '</span>' + tplCfg.param.text_vote_item_unit);
